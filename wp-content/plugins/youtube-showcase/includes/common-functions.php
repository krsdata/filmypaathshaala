			...
			if ($dtype == 'file') {
				...
				if(!empty($where) && $where == 'key'){
					...
					elseif(!empty($val) && !empty($attr_list[$ptype][$attr_id]['options'][$val])){
						$val = $attr_list[$ptype][$attr_id]['options'][$val];
					}
				}
			} else {
...
if (!function_exists('emd_load_file')) {
	function emd_load_file() {
		...
		$myapp = strtolower(preg_replace('/_PLUGIN_DIR$/','',$_POST['path']));
		require_once constant($_POST['path']) . 'assets/ext/filepicker/upload.php';
		$upload_handler = new UploadHandler(true, $_POST['field'], $_POST['extensions'],$myapp);
		die();
	}
}
if (!function_exists('emd_delete_file')) {
	function emd_delete_file() {
		...
		if(!empty($sess_files[$_POST['field']])){
			foreach($sess_files[$_POST['field']] as $kattch => $myattch){
				if($myattch['name'] == $_POST['del_file']){
					unset($sess_files[$_POST['field']][$kattch]);
				}
			}
