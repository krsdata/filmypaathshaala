...
add_action('emd_ext_set_conf','emd_form_builder_install');

function emd_form_builder_install($app){
	$app = str_replace('-','_',$app);
	emd_form_builder_save_forms_data($app);
}

add_action('emd_ext_init','emd_form_old_forms');

function emd_form_old_forms($app){
	$shc_list = get_option($app . '_shc_list',Array());
	if(!empty($shc_list['forms'])){
		foreach($shc_list['forms'] as $fkey => $fval){
			add_shortcode($fkey,'emd_old_forms_shc');
		}
	}
}

function emd_old_forms_shc($atts,$content,$tag){
	if(!empty($tag)){
		$fid = get_option('emd_form_id_' . $tag,0);
		if(!empty($fid)){
			echo do_shortcode("[emd_form id='" . $fid . "']");
		}
	}
}

if (!is_admin() || (defined('DOING_AJAX') && DOING_AJAX)) {
	add_filter('posts_where', 'emd_form_builtin_posts_where', 10, 2);
}
add_action('emd_ext_admin_enq', 'emd_form_builder_admin_enq', 10, 2);

function emd_form_builder_admin_enq($app,$hook){
	if(preg_match('/page_' . $app . '_forms$/',$hook)){
		...
		if(!empty($_GET['edit']) && $_GET['edit'] == 'layout'){
			...
			wp_enqueue_style('form-builder-css', $dir_url . 'includes/emd-form-builder-lite/css/emd-form-builder.css');
			$builder_vars['ajax_url'] = admin_url('admin-ajax.php');
			$builder_vars['exit_url'] = admin_url('admin.php?page=' . $_GET['page']);
		...
		elseif(!empty($_GET['edit']) && $_GET['edit'] == 'settings'){
			...
			wp_enqueue_script('clipboard-js', $dir_url . 'assets/js/clipboard.min.js', array() , '');
			wp_enqueue_script('form-list',$dir_url . 'includes/emd-form-builder-lite/js/emd-form-list.js');
		}
	}
}
...
function emd_form_builder_save_forms_data($app){
				...
				foreach($fsettings as $skey => $sval){
					...
					if(in_array($skey,$non_field_confs) && !is_array($sval)){
						...
						elseif(!empty($txn_list[$myform['entity']][$skey])){
							$myfield[$skey]['label'] = $txn_list[$myform['entity']][$skey]['label'];
...
add_action('emd_show_forms_page','emd_show_forms_page',1);
/**
 ...
 */
function emd_show_forms_page($app){
	global $title;
	echo '<div class="wrap">';
	...
	if(!empty($_POST['submit']) && !empty($_POST['submit_settings'])){
		emd_form_builder_save_settings($app);
	}
	elseif(!empty($_GET['edit']) && $_GET['edit'] == 'layout' && !empty($_GET['form_id'])){
		emd_form_builder_layout($app,$_GET['form_id']);
	}
	elseif(!empty($_GET['edit']) && $_GET['edit'] == 'settings' && !empty($_GET['form_id'])){
		emd_form_builder_settings('edit',$app,$_GET['form_id']);
	}
	else {
...
function emd_form_builder_layout($app,$form_id,$from='admin'){
	...
	emd_form_builder_fields($app,$fentity,$fcontent);
	echo '</div>
		<div class="emd-form-builder-fields-settings" style="display:none;">';
	emd_form_builder_get_form_field_settings($app,$fentity,$fcontent);
	echo '</div>';
	echo '</div>';
	echo '<div class="emd-form-builder-content-wrap">
		<div class="emd-form-builder-content">';
	emd_form_builder_get_form_layout($app,$fentity,$myform->post_title,$fcontent);
	echo '</div>';
	echo '</div>';
	echo '</div>';
	echo '</form>';
...
function emd_form_builder_fields($app,$fentity,$fcontent){
	...
	if(!empty($attr_list[$fentity])){
		...
		foreach($attr_list[$fentity] as $kattr => $vattr){
			...
			if(!preg_match('/^wpas_/',$kattr) && !(!empty($vattr['uniqueAttr']) && $vattr['display_type'] = 'hidden')){
	...
	if(!empty($txn_list[$fentity])){
		...
		foreach($txn_list[$fentity] as $ktxn => $vtxn){
			...
			echo ' emd-attr" id="' . $ktxn . '-btn" data-field="' . $ktxn . '" title="' . $titl . '">' . $vtxn['label'] . '</button>';
		}
		echo '</div>';
		echo '</div>';
	}	
	if(!empty($rel_list)){
		...
		if(!empty($rels)){
			...
			foreach($rels as $myrel){
				...
				elseif(!in_array($myrel['key'],$glob_list)){
					echo ' upgrade-pro';
					$titl = __('Drag this field to a row - available in premium edition.','emd-plugins');
				}
...
function emd_form_builder_get_form_layout($app,$fentity,$ftitle,$fcontent){
	$attr_list = get_option($app . '_attr_list',Array());
	$ent_list = get_option($app . '_ent_list',Array());
	$txn_list = get_option($app . '_tax_list', Array());
	$rel_list = get_option($app . '_rel_list', Array());
					...
					foreach($crow as $fcount => $field){
						foreach($field as $kfield => $cfield){
							if(!empty($cfield['show']) && !in_array($kfield, Array('login_box_password','login_box_reg_password','login_box_reg_confirm_password','login_box_reg_username'))){ 
								...
								echo '<span class="emd-form-field-info">' . __('Drag to reorder / Click for settings','emd-plugins') . '</span>';
								...
								if($kfield == 'hr'){
									emd_form_builder_layout_hr($kfield);
								}
								elseif(preg_match('/^html/',$kfield)){
									emd_form_builder_layout_html($kfield,$cfield);
								}
								else {
									if(in_array($kfield,array_keys($attr_list[$fentity]))){
										$cfield['display_type'] =  $attr_list[$fentity][$kfield]['display_type'];
									}
									emd_form_builder_layout_field_top_bottom($kfield,$cfield,'top');
									if(in_array($kfield,Array('blt_title','blt_content','blt_excerpt'))){
										echo emd_form_builder_blt_fields($kfield,$cfield);
									}
									elseif(preg_match('/^login_box_/',$kfield)){
										echo emd_form_builder_login_box($kfield,$cfield);
									}
...
function emd_form_builder_layout_html($kfield,$cfield){
	echo '<div class="form-group">
		<input type="hidden" name="layout[]" value="' . $kfield . '">
		<input type="text" name="' . $kfield . '" id="' . $kfield . '" class="text input-md form-control html-code" placeholder="' . __('HTML','emd-plugins') . '" disabled/>
		</div>';
}
function emd_form_builder_layout_hr($kfield){
	echo '<hr class="emd-form-row-hr">
		<input type="hidden" name="layout[]" value="hr">';
}
function emd_form_builder_layout_field_top_bottom($kfield,$cfield,$loc){
	if(!empty($cfield['display_type']) && in_array($cfield['display_type'], Array('checkbox')) && $loc == 'top'){
		...
		if(!empty($cfield['display_type']) && in_array($cfield['display_type'],Array('checkbox'))){
			echo 'form-check-label" for="' . $kfield . '">';
		}
		else {
			echo 'control-label" for="' . $kfield . '">';
		}
...
function emd_form_builder_blt_fields($kfield,$cfield){
	if($kfield == 'blt_title'){
		$blt_lay = '<input type="text" name="' . $kfield . '" id="' . $kfield . '" class="text input-md form-control" placeholder="' . $cfield['placeholder'] . '" disabled/>';
	}	
	else {
...
function emd_form_builder_attr_fields($kfield,$cfield){
	$attr_lay = '';
	switch($cfield['display_type']){
		case 'select':
		case 'select_advanced':
			$attr_lay .= '<div class="dropdown">';
			$attr_lay .= '<input type="text" name="' . $kfield . '" id="' . $kfield . '" class="text input-md form-control" placeholder="' . $cfield['placeholder'] . '" disabled>';
		...
		case 'wysiwyg':
			...
			break;
		case 'checkbox':
			$attr_lay .= '<input type="checkbox" name="' . $kfield . '" id="' . $kfield . '" class="checkbox input-md form-control" disabled>';
			break;
		case 'radio':
			if(!empty($cfield['options'])){
				foreach($cfield['options'] as $kopt => $vopt){
					$attr_lay .= '<div class="form-check">';
					$attr_lay .= '<input type="radio" name="' . $kfield . '" id="' . $kfield . '_' . $kopt . '" class="radio input-md form-control" disabled>';
		...
		case 'checkbox_list':
			if(!empty($cfield['options'])){
				foreach($cfield['options'] as $kopt => $vopt){
					$attr_lay .= '<div class="form-check">';
					$attr_lay .= '<input type="checkbox" name="' . $kfield . '" id="' . $kfield . '_' . $kopt . '" class="checkbox input-md form-control" disabled>';
					$attr_lay .= '<label class="form-check-label" for="' . $kfield . '_' . $kopt . '">' . $vopt  . '</label>';
					$attr_lay .= '</div>';
				}
			}
...
function emd_form_builder_txn_fields($kfield,$cfield){
	$txn_lay = '<div class="dropdown">';
	$txn_lay .= '<input type="text" name="' . $kfield . '" id="' . $kfield . '" class="text input-md form-control" placeholder="' . $cfield['placeholder'] . '" disabled>';
	$txn_lay .= '<div class="emd-arrow"></div>';
	$txn_lay .= '</div>';
	$txn_lay .= '</div>';
	return $txn_lay;
}
function emd_form_builder_rel_fields($kfield,$cfield){
	$rel_lay = '<div class="dropdown">';
	$rel_lay .= '<input type="text" name="' . $kfield . '" id="' . $kfield . '" class="text input-md form-control" placeholder="' . $cfield['placeholder'] . '" disabled>';
	$rel_lay .= '<div class="emd-arrow"></div>';
	$rel_lay .= '</div>';
	$rel_lay .= '</div>';
	return $rel_lay;
}
add_action('wp_ajax_emd_form_builder_get_field', 'emd_form_builder_get_field');
function emd_form_builder_get_field(){
	if(!empty($_POST['app']) && !empty($_POST['entity']) && !empty($_POST['field'])){	
		$app = $_POST['app'];
		$fentity = $_POST['entity'];
		$kfield = $_POST['field'];
		$attr_list = get_option($app . '_attr_list',Array());
		$ent_list = get_option($app . '_ent_list',Array());
		$txn_list = get_option($app . '_tax_list', Array());
		$rel_list = get_option($app . '_rel_list', Array());
		$cfield = emd_form_builder_get_cfield($fentity,$kfield,$attr_list,$ent_list,$txn_list,$rel_list);
		ob_start();
...
function emd_form_builder_get_form_field_settings($app,$fentity,$fcontent){
	$attr_list = get_option($app . '_attr_list');
	$txn_list = get_option($app . '_tax_list', Array());
	$rel_list = get_option($app . '_rel_list', Array());
	$ent_list = get_option($app . '_ent_list',Array());
	...
	$login_fields = Array();
	//var_dump($fcontent['layout']);
	$fields = Array('req','label','size','desc','placeholder','css_class','login_label','reg_label','username_label','password_label','redirect_link','enable_registration','reg_username_label','reg_password_label','reg_confirm_password_label');
	foreach($fcontent['layout'] as $pid => $pcont){
		if(!empty($pcont['rows'])){
			foreach($pcont['rows'] as $rid => $rcont){
				foreach($rcont as $r => $row){
	...
	if(!empty($txn_list[$fentity])){
		foreach($txn_list[$fentity] as $ktxn => $vtxn){
			if(empty($all_fields[$ktxn]['label'])){
				$all_fields[$ktxn]['label'] = $vtxn['label'];
			}
...
add_action('wp_ajax_emd_form_builder_get_page', 'emd_form_builder_get_page');
function emd_form_builder_get_page(){
	if(!empty($_GET['page_id']) && !empty($_GET['app']) && !empty($_GET['entity'])){
		...
		$playout = '<div class="emd-form-page-wrap" id="emd-form-page-' . $npid . '" data-page="' . $npid . '" style="display:none;">
			...
			<a href="#" class="emd-form-row-delete" title="' . __('Delete','emd-plugins') . '"><span class="field-icons times-circle" aria-hidden="true"></span></a>
			<span class="emd-form-row-info">' . __('Drag to reorder','emd-plugins') . '</span>
			<div class="emd-form-row-holder" data-app="' . $_GET['app'] . '" data-entity="' . $_GET['entity'] . '" data-row="0">
			<input type="hidden" name="layout[]" value="row">
			<div class="emd-form-insert-row">' . __('Drag fields here','emd-plugins') . 
			'</div>
			</div>
...
add_action('wp_ajax_emd_form_builder_get_row','emd_form_builder_get_row');
function emd_form_builder_get_row(){
...
add_action('wp_ajax_emd_form_builder_save_form','emd_form_builder_save_form');

function emd_form_builder_save_form(){
	$layout = Array();
	$fields = Array();
	if(!empty($_POST['data'])){
...
function emd_form_builder_get_cfield($fentity,$kfield,$attr_list,$ent_list,$txn_list,$rel_list){
	$rel_labels = Array();
	$cfield = Array('req' => 0);
	foreach($rel_list as $krel => $vrel){
		if($fentity == $vrel['from']){
	...
	elseif(!empty($txn_list[$fentity][$kfield])){
		$cfield['label'] = $txn_list[$fentity][$kfield]['label'];
...
add_action('wp_ajax_emd_form_builder_get_hr', 'emd_form_builder_get_hr');
function emd_form_builder_get_hr(){
	if(!empty($_GET['app']) && !empty($_GET['entity'])){
...
add_action('wp_ajax_emd_form_builder_get_html', 'emd_form_builder_get_html');
function emd_form_builder_get_html(){
	if(!empty($_GET['htmlcount']) && !empty($_GET['app']) && !empty($_GET['entity'])){
		$playout = '<div class="emd-form-row init html">
			...
			<div class="emd-form-row-holder" data-app="' . $_GET['app'] . '" data-entity="' . $_GET['entity'] . '" data-row="0">
			<input type="hidden" name="layout[]" value="row">
			<div class="emd-form-field emd-form-html emd-col" data-field="html' . $_GET['htmlcount'] . '">
			...
			<span class="emd-form-field-info">' . __('Drag to reorder / Click for settings','emd-plugins') . '</span>
			<div class="form-group">
			<input type="hidden" name="layout[]" value="html' . $_GET['htmlcount'] . '">
			<div id="html' . $_GET['htmlcount'] . '" class="emd-form-html-div"><span class="emd-html-click">' . __('Click to go to settings','emd-plugins') . '</span>
			<input type="text" class="text input-md form-control html-code" style="display:none;" placeholder="' . __('HTML','emd-plugins') . '" disabled/>
			</div>
			</div>
			</div>
			</div>
			</div>';
		$psetting = '<div class="emd-form-builder-field-settings-wrap emd-field-html' . $_GET['htmlcount'] . '" style="display:none;">
			<div class="emd-form-builder-field-setting html">
			<label for="emd-field-html">' . __('HTML','emd-plugins') . '</label>
			<textarea name="fields[html' . $_GET['htmlcount'] . '][value]" id="emd-fbhtml-' . $_GET['htmlcount'] . '" class="emd-form-builder-field-html" rows=3></textarea>
			</div>';
		wp_send_json_success(array('row' => $playout,'setting' => $psetting));
	}
}
function emd_form_builder_login_box($kfield,$cfield){
	if($kfield == 'login_box_username'){
		$lay = '<div class"login_register">
			<input type="text" name="' . $kfield . '" id="' . $kfield . '" class="text input-md form-control" placeholder="' . __('Login / Register Box','emd-plugins') . '" disabled/>';
		$lay .= '</div></div>';
	}
	else {
		$lay = '';
