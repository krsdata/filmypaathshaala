 ...
 */
if (!function_exists('emd_form_builder_search_form')) {
	function emd_form_builder_search_form($myapp, $fcontent) { 
		$layout = '';
		...
		if (!empty($fcontent['settings']['display_records']) || !empty($attrs) || !empty($txns) || !empty($rels) || !empty($blts) || ($paged != 0 && !empty($sess_form_args))) {
			...
			$res_layout =  emd_form_builder_set_shc('', $args, $fields, $paged);
 ...
 * @return array $ret
 */
if (!function_exists('emd_form_builder_submit_form')) {
	function emd_form_builder_submit_form($app, $fcontent) {
		$user_conf = Array();
		$entity_post = Array();
		$entity_fields = Array();
		$txn_fields = Array();
		...
		if(!empty($fcontent['settings']['honeypot']) && !empty($_POST)){	
			$honey_pre = $fcontent['name'] . $_POST['emd_step'] . '_';
		...
		$ret_attrs = emd_form_builder_check_attr($app,$fcontent['entity'],$ent_list,$attr_list);
		if (!empty($ret_attrs['file_attrs'])) {
			$sess_name = strtoupper($app);
			$session_class = $sess_name();
			$sess_uploads = $session_class->session->get('uploads');
			emd_form_builder_set_file_upload($sess_uploads);
		}
...
if (!function_exists('emd_form_builder_set_shc')) {
	function emd_form_builder_set_shc($atts, $args = Array() , $myfields, $pageno = 1, $shc_page_count = 0){
...
if (!function_exists('emd_form_builder_cond_vars')) {
	function emd_form_builder_cond_vars($layout,$fentity,$attr_list,$txn_list){
		$cond_arr= Array();
...
if (!function_exists('emd_form_builder_req_hide_vars')) {
	function emd_form_builder_req_hide_vars($layout){
		$req_arr= Array();
		foreach($layout as  $kpage => $mypage){
			foreach($mypage['rows'] as $myrow){
				foreach($myrow as $fcount => $field){
...
add_filter('emd_ext_parse_tags', 'emd_form_builder_parse_tags', 10, 3 );
if (!function_exists('emd_form_builder_parse_tags')) {
	function emd_form_builder_parse_tags($message,$pid,$app){
		if(!empty($pid)){
			$mypost = get_post($pid);
...
add_action('init', 'emd_form_builder_user_actions');

function emd_form_builder_user_actions(){
	if(!empty($_GET['emd_action']) && $_GET['emd_action'] == 'verify' && !empty($_GET['id']) && !empty($_GET['app']) && !empty($_GET['token'])){
		...
		if($token == $_GET['token']){
			...
			$ent_list = get_option($_GET['app'] . '_ent_list', Array());
			$entity = get_post_type($_GET['id']);
			$user_key = $ent_list[$entity]['user_key'];
			wp_update_post(Array('ID'=>$_GET['id'],'post_status'=>'publish'));
			$user_id = get_post_meta($_GET['id'],$user_key,true);
			if(!empty($user_id)){
			...
			do_action('emd_after_verify_token',$_GET['app'], $_GET['id']);
...
function emd_get_form_shc($fname){
...
function emd_form_builder_check_attr($layout,$fentity,$ent_list,$attr_list,$txn_list=Array(),$rel_list=Array()){
	$ret = Array();
	if(!empty($layout)){
		foreach($layout as $kpage => $mypage){
			foreach($mypage['rows'] as $myrow){
...
function emd_form_builder_set_file_upload($sess_uploads) {
	if (!empty($_FILES)) {
...
function emd_form_builder_unset_file_upload($session_class) {
	$sess_uploads = $session_class->session->get('uploads');
	if (!empty($sess_uploads)) {
		$session_class->session->set('uploads','');	
	}
}
function emd_form_builder_search_results($fields,$type,$pid){
	...
	if(!empty($fields['form_res_fields'])){
		foreach($fields['form_res_fields'] as $myfield){
			...
			elseif(!empty($txn_list[$fields['class']][$myfield])){
				...
				if($type == 'content' && !empty($pid)){
					$results[] = emd_get_tax_vals($pid,$myfield,1);
				}
			}
	...
	elseif($fields['shc'] == 'adv_table'){
		if($type == 'header'){
			...
			if($fields['adv_table']['adv_show_export']){
				...
				<li data-type="">
				<a href="#"><i class="fa fa-fw fa-check"></i>' . __('Current Page', 'empd-pro') . '</a>
				</li>
				<li data-type="all">
				<a href="#"><i class="fa fa-fw"></i>' . __('All', 'empd-pro') . '</a>
				</li>
				<li data-type="selected">
				<a href="#"><i class="fa fa-fw"></i>' . __('Selected', 'empd-pro') . '</a>
				</li>
				</ul>';
			}
			$ret .= '</div>
...
add_action('wp_ajax_emd_form_builder_pagenum', 'emd_form_builder_pagenum');
function emd_form_builder_pagenum() {
	$response = false;
		...
		if(!empty($form_posts)){
			foreach($form_posts as $myform_post){
				...
				if($fcontent['name'] == $myform){
					...
					$response =  emd_form_builder_set_shc('', $sess_form_args, $fields, $pageno);
				}
			}
		}
	}
...
function emd_form_builtin_posts_where($where, $wp_query) {
