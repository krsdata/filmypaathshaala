...
function emd_form_builder_settings($type,$app,$form_id=0,$result = ''){
	if($type == 'edit'){
	...
	foreach(array_keys($tabs) as $tab){
		emd_form_builder_tab($type,$tab,$app,$active_tab,$fcontent);
	}
	echo submit_button(); 
	echo '</form>';
	echo '</div>';
}
function emd_form_builder_tab($type,$mytab,$app,$active_tab,$fcontent){
	switch($mytab){
		case 'general':
		...
		case 'confirm':
			...
			if(!empty($forms)){
				foreach($forms as $myform){
					...
					if($mycont['name'] != $fcontent['name'] && $mycont['type'] == 'submit'){
		...
		case 'search':
			...
			$temp_options = Array('simple_table' => __('Basic HTML Table','emd-plugins'),'adv_table'=>__('Advanced Datagrid Table','emd-plugins'),'cust_table' => __('Custom','emd-plugins'));
			$tab_fields = Array(
	...
	echo emd_form_builder_show_setting($tab_fields,$fcontent);
	echo '</div>';
	echo '</div>';
}
function emd_form_builder_show_setting($tab_fields,$fcontent){
	if(!empty($tab_fields)){
...
function emd_form_builder_save_settings($app){
	if(!empty($_POST['stype']) && $_POST['stype'] == 'edit' && !empty($_POST['id']) && !empty($_POST['settings'])){
		...
		if(!is_wp_error($res)){
			emd_form_builder_settings('edit',$app,$_GET['form_id'],'success');
		}
		else {
			emd_form_builder_settings('edit',$app,$_GET['form_id'],'error');
		}
	}
	else if(!empty($_POST['stype']) && $_POST['stype'] == 'new' && empty($_POST['id']) && !empty($_POST['settings'])){
		$fcontent = Array();
		$fcontent['type'] = $_POST['settings']['type'];
		$fcontent['app'] = $_POST['app'];
		$fcontent['name'] = strtolower(str_replace(" ","_",$_POST['settings']['title']));

		$fcontent['entity'] = $_POST['settings']['entity'];
		$fcontent['settings'] = $_POST['settings'];
		$fcontent['source']  = 'user';
		$fcontent['layout']  = Array();
		$form_data = array(
			'post_title' => $_POST['settings']['title'],
			'post_type' => 'emd_form',
			'post_status' => 'publish',
			'post_content' => wp_slash(json_encode($fcontent,true)),
		);
		if ($id = wp_insert_post($form_data)) {
			emd_form_builder_settings('edit',$app,$id,'success');
		}
		else {
			emd_form_builder_settings('new',$app,0,'error');
		}
	}
	else {
		emd_form_builder_settings('edit',$app,$_GET['form_id'],'error');
