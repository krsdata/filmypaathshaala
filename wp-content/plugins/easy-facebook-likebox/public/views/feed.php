 ...
 * @author    Your Name <malikdanishbusiness@gmail.com>
 * @link      http://maltathemes.com
...
if(!isset( $access_token ) or !isset($instance['fb_appid']) ){
	...
	$access_token = get_option('efbl_access_token', false);

	if(empty($access_token)) $access_token = '222116127877068|4223b75bbcf692242b677db0c08d9b56';
...
$jws_api_url = 'https://graph.facebook.com/v3.2/' .$page_id. '/?fields=id,name,posts{full_picture,permalink_url,message,description,created_time,likes,comments,reactions,from,type,link,message_tags,shares,story_tags,name,object_id,source,picture}&access_token='. $access_token . '&limit=' . $post_limit . '&locale=en_us';
...
if( !empty($fbData->posts->data) ) {
	...
	echo '<div class="efbl_feed_wraper" id="efbl_feed_'.$rand_id.'">';
		...
		foreach($fbData->posts->data as $story){
	...
	echo '<p>Error:'.$fbData->error->message.'</p>';